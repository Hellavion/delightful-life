...
<head>
    ...
    <title>Редактирование заказа {{ $order->order_number }} - Административная панель</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-xl font-semibold text-gray-900">Редактирование заказа</h1>
                </div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Редактирование заказа {{ $order->order_number }}</h1>
                <p class="mt-2 text-gray-600">Изменение информации о заказе</p>
            </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" 
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $order->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   name="dimensions" 
                                   id="dimensions"
                                   value="{{ old('dimensions', $order->dimensions) }}"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="deposit" 
                                   id="deposit"
                                   value="{{ old('deposit', $order->deposit) }}"
                                   min="0"
                                   step="0.01"
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center">
                                <input type="checkbox" 
                                       ...
                                       id="deposit_paid"
                                       value="1"
                                       @checked(old('deposit_paid', $order->deposit_paid))
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" 
                                       ...
                                       value="1"
                                       @checked(old('full_payment_received', $order->full_payment_received))
                                       class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="status" 
                                ...
                                @foreach($statuses as $key => $label)
                                    <option value="{{ $key }}" @selected(old('status', $order->status) === $key)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <textarea name="notes" 
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('notes') border-red-500 @enderror">{{ old('notes', $order->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            <span class="text-gray-700">Номер заказа:</span>
        ...
        <div class="flex items-center justify-between pt-6 border-t border-gray-200">
            <div>
                <form method="POST" action="{{ route('admin.orders.destroy', $order) }}" 
                      onsubmit="return confirm('Вы уверены, что хотите удалить этот заказ? Это действие нельзя отменить.')"
                      class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
...
document.getElementById('price').addEventListener('input', function() {
    ...
    // Только если поле депозита пустое или содержит значение, рассчитанное как 30% от предыдущей цены
    const currentDeposit = parseFloat(depositInput.value) || 0;
    if (price > 0 && (currentDeposit === 0 || Math.abs(currentDeposit - (price * 0.3)) < 1)) {
