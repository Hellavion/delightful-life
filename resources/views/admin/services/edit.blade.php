...
<head>
    ...
    <title>Редактирование услуги: {{ $service->name }} - Административная панель</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-xl font-semibold text-gray-900">Редактирование: {{ $service->name }}</h1>
                </div>
                
                <div class="flex items-center space-x-4">
                    <a href="{{ route('admin.services.show', $service) }}" class="text-blue-600 hover:text-blue-800">
                        Просмотр
                    </a>
        ...
        <div class="bg-white rounded-lg shadow">
            <form method="POST" action="{{ route('admin.services.update', $service) }}" class="space-y-6 p-6">
                ...
                <div class="border-b border-gray-200 pb-6">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="description" 
                                  ...
                                  rows="4"
                                  required
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="process_description" 
                                  ...
                                  rows="4"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('process_description') border-red-500 @enderror">{{ old('process_description', $service->process_description) }}</textarea>
                        ...
                        <div class="md:col-span-3">
                            ...
                            <select id="pricing_type" 
                                ...
                                @foreach($pricingTypes as $value => $label)
                                    <option value="{{ $value }}" {{ old('pricing_type', $service->pricing_type) === $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                ...
                @if($service->orders()->exists())
                    <div class="border-b border-gray-200 pb-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Связанные заказы</h3>
                        <div class="bg-blue-50 border border-blue-200 rounded-md p-4">
                            <p class="text-sm text-blue-800">
                                С этой услугой связано заказов: <strong>{{ $service->orders()->count() }}</strong>
                            </p>
                            <p class="text-xs text-blue-600 mt-1">
                                При изменении цен и описания учтите, что это может повлиять на существующие заказы.
                            </p>
                        </div>
                    </div>
                @endif
